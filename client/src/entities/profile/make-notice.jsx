...
const MakeNotice = () => {
  ...
  useAutosizeTextArea(textAreaRef.current, makeController.textarea, wrapRef.current)
  if (profileController.changeID === 0 && loginStore.user.id === profileController.pageID) {
    return (
      <>
        <p className={cl.errMakeNotice}>{makeController.error}</p>
        <div ref={wrapRef} id={makeController.showMakeFull ? cl.handleBorderBottom : ''} className={cl.makeNoticeWrap}>
          <Image className={cl.avatar} src={loginStore.user.avatar} alt='anidale' />
          ...
          <InputFileImage id='noticeImages' onChange={(e) => makeController.selectFiles(e.target.files)} />
          <label className={cl.label} htmlFor='noticeImages'>
            <Icon id='addphoto' className={cl.labelSvg} />
          </label>
          <Icon disabled={true} id='send' className={cl.labelSvg} onClick={makeController.sendNotice} />
        </div>
        {makeController.showMakeFull && (
          <div id={makeController.sendLoading ? cl.sendLoading : ''} ref={fullAddRef} className={cl.showAddFull}>
