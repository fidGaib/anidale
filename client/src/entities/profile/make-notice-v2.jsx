...
const MakeBoticeV2 = () => {
  ...
  if (profileController.changeID === 0 && loginStore.user.id === parseInt(params.id)) {
    return (
      <>
        <p className={cl.errMakeNotice}>{makeNoticeController.error}</p>
        <div id={makeNoticeController.images.length ? cl.handleBorderBottom : ''} className={cl.makeNoticeWrap}>
          ...
          <Icon id='send' className={cl.labelSvg} onClick={makeNoticeController.sendNotice} />
        </div>
